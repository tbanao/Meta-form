# app.py  (2025-06-12  + _fbc/_fbp 自動補齊 & eventCallback)
...
/* ---------- 自動補 _fbc / _fbp ---------- */
function getC(name){{return (document.cookie.match('(^|;) ?'+name+'=([^;]*)(;|$)')||[])[2]||'';}}
function setC(n,v){{document.cookie=n+'='+v+';path=/;SameSite=Lax';}}
(function ensureFbCookies(){{
  if(!getC('_fbp')){{
    setC('_fbp','fb.1.'+Math.floor(Date.now()/1000)+'.'+Math.floor(Math.random()*1e16));
  }}
  const qs=new URLSearchParams(location.search);const fbclid=qs.get('fbclid');
  if(fbclid&& !getC('_fbc')){{
    setC('_fbc','fb.1.'+Math.floor(Date.now()/1000)+'.'+fbclid);
  }}
}})();
/* ---------------------------------------- */
...
function beforeSubmit(e){{
  ...
  setTimeout(()=>e.target.submit(),800);   // 備援
}}
</script></head><body>
...
<form method="post" action="/submit" onsubmit="beforeSubmit(event)">
  ...
  性別：<select name="gender">
          <option value="female">女性</option><option value="male">男性</option></select><br>
...
# ─── Routes ──────────────────────────────────────
@app.route('/'); app.route('/healthz')(lambda:("OK",200))
...
def submit():
    ...
    # 若仍無 _fbc → 嘗試從 Referer 的 fbclid 組
    ...
    # ---- Excel 備份 ----
    file=BACKUP_DIR/f"{name}_{ts}.xlsx"
    from openpyxl import Workbook; wb=Workbook(); ws=wb.active
    ws.append(["姓名","Email","電話","價值","提交時間"]); ws.append([name,email,phone,price,ts])
    wb.save(file)

    # ---- user_profile_map ----
    if not Path(PROFILE_MAP).exists():
        with open(PROFILE_MAP,"wb") as f: pickle.dump({},f)
    with locked(PROFILE_MAP,"rb") as f: profs=pickle.load(f)
    ...
    prof=profs.get(key,{}); city=prof.get("ct") or random.choice(city_choices)
    prof.update({"fn":fn,"ln":ln,"em":email,"ph":phone,"ct":city,"zp":zp_map[city]})
    profs[key]=prof
    with locked(PROFILE_MAP,"wb") as f: pickle.dump(profs,f)

    # ---- CAPI ----
    ud={"external_id":sha(key),"fn":sha(fn),"ln":sha(ln),"em":sha(email),"ph":sha(phone),
        "ct":sha(city),"zp":sha(zp_map[city]),
        "client_ip_address":request.remote_addr or "",
        "client_user_agent":request.headers.get("User-Agent",""),
        "fbc":fbc,"fbp":fbp}

    custom={"currency":CURRENCY,"value":price,"submit_time":ts,
            "gender":gender,"birthday":bday,"satisfaction":sat,"suggestion":sug}
    ...
    if not resp.ok:
        with open(RETRY_FILE,"a",encoding="utf-8") as f: f.write(json.dumps(payload,ensure_ascii=False)+"\n")

    # ---- email ----
    msg=EmailMessage(); msg["Subject"]="新客戶表單"; msg["From"]=os.environ["FROM_EMAIL"]
    msg["To"]=",".join([os.environ["TO_EMAIL_1"],os.environ["TO_EMAIL_2"]])
    msg.set_content(f"{name} 送出表單 (價值 {price})")
    with open(file,"rb") as f: msg.add_attachment(f.read(),
        maintype="application",subtype="vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        filename=file.name)
    with smtplib.SMTP_SSL("smtp.gmail.com",465) as s:
        s.login(os.environ["FROM_EMAIL"],os.environ["EMAIL_PASSWORD"]); s.send_message(msg)
