...
def mask(text, keep=2):
    return text[:keep] + "*"*(max(0, len(text)-keep-2)) + text[-2:] if text else ""

hash_sha256 = lambda t: hashlib.sha256(t.encode()).hexdigest() if t else ""

def normalize_phone(p):
    digits = re.sub(r"[^\d]", "", p)
    return digits if digits.startswith("8869") else ("886"+digits.lstrip("0") if digits.startswith("09") else digits)
...
def verify_csrf(tok): return tok and tok == session.get("csrf")

def save_to_excel(data: dict, path: Path):
    wb = Workbook(); ws = wb.active
    ws.append(list(data.keys())); ws.append(list(data.values()))
    wb.save(path)

def send_email_with_attachment(path: Path, raw: dict):
    msg = EmailMessage()
    msg["Subject"] = "新客戶表單回報"
    msg["From"] = FROM_EMAIL
    msg["To"] = TO_EMAILS
...
button:hover{background:#376ad8}h2{margin-top:0;color:#34495e}
</style>
<script>
...
@app.route('/')
def index():
    return render_template_string(HTML_FORM, csrf=generate_csrf(),
                                  cities=CITIES, PIXEL=PIXEL_ID)
...
def submit():
    ...
    save_to_excel({"姓名":name,"Email":email,"電話":phone,"城市":city,"提交時間":ts}, backup)
    ...
    if city:
        ud["ct"] = hash_sha256(city)
        ud["zp"] = hash_sha256(prof["zp"])
    ...
    try:
        ...
        print("❌ CAPI 呼叫失敗:", e); traceback.print_exc()
