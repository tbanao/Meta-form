...
• 抽中後帶入真實 IP/UA，隨機 fbc/fbp/price/event_id
• user_data 哈希所有 PII：fn/ln, em, ph, ge, country, dob
...
window.addEventListener('DOMContentLoaded',()=>{
  const now=newDate(),y=now.getFullYear();
  for(let i=y-90;i<=y;i++){const o=newOption(i,i);if(i===y-25)o.selected=true;byear.appendChild(o)}
  for(let i=1;i<=12;i++)bmonth.appendChild(newOption(i,i.toString().padStart(2,'0')));
  for(let i=1;i<=31;i++)bday.appendChild(newOption(i,i.toString().padStart(2,'0')));
...
def submit():
    ...
    with locked(USER_PROFILE_MAP_PATH,"a+b") as f:
        ...
        for k in filter(None,[
            ...
            mp[k]=u
        f.seek(0);pickle.dump(mp,f);f.truncate()
        logging.info("Map 更新：%s",list(mp.keys()))
    ...
    pv={"event_name":"PageView","event_time":ts-random.randint(60,300),
        "event_id":f"{eid}_pv","action_source":"website","user_data":ud}
    purchase={"event_name":"Purchase","event_time":ts,"event_id":eid,
    ...
    try:
        ...
        body="\n".join([
            ...
            f"【滿意度】{d['satisfaction']}",f"【建議】{d['suggestion']}"
        ])
        msg=EmailMessage()
        msg["Subject"]="新客戶表單回報"
